...
## Стандартный stdio-режим без конфигурационного файла

MCP-клиент запускает короткую команду и задаёт токен в `env` только для дочернего процесса:

```shell
notification-mcp
```

Эквивалентная полная команда остаётся доступной для отладки:

```shell
notification-mcp serve --transport stdio
```

Процесс должен получить `TELEGRAM_BOT_TOKEN` через пару `ключ → значение` в конфигурации самого
MCP-хоста. Системная переменная ОС не нужна. Для нестандартного имени в полной команде доступен
`--token-env MY_TELEGRAM_TOKEN`.

Создаётся один канал `personal` типа Telegram и маршрут по умолчанию. Конфигурационный файл
...
`TELEGRAM_BOT_TOKEN`. Универсальная запись `command`/`env` приведена в
[инструкции подключения](clients.md#универсальный-stdio-профиль).

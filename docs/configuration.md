...
## Режим без конфигурационного файла

Команда `serve --transport stdio` может построить минимальные настройки непосредственно из
переменной `TELEGRAM_BOT_TOKEN`:

```shell
notification-mcp serve --transport stdio
```

Этот режим включается, когда токен присутствует в окружении, а `--config` и
`NOTIFICATION_MCP_CONFIG` не заданы. Для нестандартного имени укажите его явно:

```shell
notification-mcp serve --transport stdio --token-env MY_TELEGRAM_TOKEN
```

Создаётся один канал `personal` типа Telegram и маршрут по умолчанию на него. Остальные параметры
доставки получают стандартные значения. Конфигурационный файл не читается и не создаётся;
SQLite-база располагается в `notification-mcp/var/notifications.sqlite3` внутри пользовательской
папки приложения. Эта база совместима с обычным режимом, поэтому одновременно запускать два
процесса нельзя.

Явный `--config` или `NOTIFICATION_MCP_CONFIG` всегда включает файловый режим даже при наличии
`TELEGRAM_BOT_TOKEN`. Настройка передачи переменной со стороны MCP-клиента описана в
[инструкции подключения](clients.md#codex-токен-из-окружения-без-сервисного-конфига).
